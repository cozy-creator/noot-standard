module noot_examples::degods {
    ...
    use std::option;
    use std::vector;
    use noot::noot::{Self, WrappedNoot};
    ...
    public fun craft(
        ...
        owner: address,
        dispenser: &mut Dispenser<SUI, Traits>,
        ctx: &mut TxContext): WrappedNoot<Degods, Market>
    {
        ...
        let (noot, transfer_cap) = noot::craft<Degods, Traits>(Degods {}, option::some(owner), &data, ctx);
        let wrapped_noot = royalty_market::wrap(noot, transfer_cap);
